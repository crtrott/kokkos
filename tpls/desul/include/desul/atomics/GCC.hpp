...
#define DESUL_GCC_INTEGRAL_OP_ATOMICS(MEMORY_ORDER, MEMORY_SCOPE)                      \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_fetch_add(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_fetch_add(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_fetch_sub(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_fetch_sub(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_fetch_and(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_fetch_and(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_fetch_or(   \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_fetch_or(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);        \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_fetch_xor(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_fetch_xor(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_fetch_nand( \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_fetch_nand(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);      \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_add_fetch(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_add_fetch(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_sub_fetch(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_sub_fetch(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_and_fetch(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_and_fetch(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_or_fetch(   \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_or_fetch(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);        \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_xor_fetch(  \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
    return __atomic_xor_fetch(dest, value, GCCMemoryOrder<MEMORY_ORDER>::value);       \
  }                                                                                    \
  template <typename T>                                                                \
  typename std::enable_if<std::is_integral<T>::value, T>::type host_atomic_nand_fetch( \
      T* const dest, T value, MEMORY_ORDER, MEMORY_SCOPE) {                            \
...
host_atomic_exchange(T* const dest,
