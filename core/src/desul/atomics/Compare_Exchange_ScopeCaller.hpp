...
T atomic_exchange(
    T* dest, Impl::dont_deduce_this_parameter_t<const T> value, MemoryOrder, MemoryScopeCaller) {
  T return_val = *dest;
  *dest = value;
  return return_val;
}
...
T atomic_compare_exchange(
    T* dest, Impl::dont_deduce_this_parameter_t<const T> compare,
    Impl::dont_deduce_this_parameter_t<const T> value, MemoryOrder, MemoryScopeCaller) {
  T current_val = *dest;
