 ...
 public:
  ...
  inline __tile__ void operator()() const {
    if constexpr (Policy::rank() == 1) {
      m_functor(cuda::tiles::bid().x);
    } else if constexpr (Policy::rank() == 2) {
      size_t bidx = cuda::tiles::bid().x;
      m_functor(bidx / m_extent[1], bidx % m_extent[1]);
    } else if constexpr (Policy::rank() == 3) {
      size_t bidx = cuda::tiles::bid().x;
      m_functor(bidx / (m_extent[1] * m_extent[2]),
                (bidx / m_extent[2]) % m_extent[1], bidx % m_extent[2]);
    } else {
      Kokkos::abort("Unsupported Rank\n");
    }
  }
  ...
  ParallelFor(const FunctorType& arg_functor, Policy arg_policy)
    ...
    for (array_index_type i = 0; i < Policy::rank(); ++i) {
      if constexpr (Policy::inner_direction == Iterate::Left) {
        ...
        m_extent[i] = m_upper[Policy::rank - 1 - i] -
                      m_policy.lower[Policy::rank - 1 - i];
      }
    }
  }
};
